...
def get_perspective_transform_method_a(src_pts, dst_pts):
    ...
    # The right side of the equation should be the dest points
    B = np.array([point for pair in zip(dst_norm[:, 0], dst_norm[:, 1]) for point in pair], dtype=np.float32)
...
for file_name, extension in image_files.items():
    ...
    # Extract original and transformed points for affine transformation
    affine_src_points = affine_data[:, :2]
    affine_dst_points = affine_data[:, 2:4]

    # Extract original and transformed points for perspective transformation
    perspective_src_points = perspective_data[:, :2]
    perspective_dst_points = perspective_data[:, 2:4]
    
    # Convert to float32 for numerical stability
    affine_src_points = affine_src_points.astype(np.float32)
    affine_dst_points = affine_dst_points.astype(np.float32)
    perspective_src_points = perspective_src_points.astype(np.float32)
    perspective_dst_points = perspective_dst_points.astype(np.float32)
    
    # Get built-in transformation matrices using minimal correspondences
    affine_transform_builtin = cv.getAffineTransform(affine_src_points[:3], affine_dst_points[:3])
    perspective_transform_builtin = cv.getPerspectiveTransform(perspective_src_points[:4], perspective_dst_points[:4])
    
    # Apply our transformation methods
    # Method A: minimal number of correspondences
    affine_transform_method_a = get_affine_transform_method_a(affine_src_points, affine_dst_points)
    affine_image_method_a = warp_affine(original_image, affine_transform_method_a)

    perspective_transform_method_a = get_perspective_transform_method_a(perspective_src_points, perspective_dst_points)
    ...
    affine_transform_method_b = get_affine_transform_method_b(affine_src_points, affine_dst_points)
    affine_image_method_b = warp_affine(original_image, affine_transform_method_b)

    perspective_transform_method_b = get_perspective_transform_method_b(perspective_src_points, perspective_dst_points)
